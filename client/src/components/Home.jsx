        ...
        <div className="sm:w-[288px] w-full rounded-[15px] bg-slate-50 cursor-pointer">
          <div className="flex p-4">
            <div>
              <div className="block">
                <Link to={`/chit/${id}`}>
                  <h3 className="font-epilogue font-semibold text-xl text-[#505050] text-left leading-[26px] truncate">{title}</h3>
                </Link>
              ...
              <div className="flex justify-between flex-wrap mt-[15px] gap-2">
                <div className="flex flex-col">
                  ...
                  <p className="mt-[3px] font-epilogue font-normal text-md leading-[18px] text-[#808191] sm:max-w-[120px] truncate">
                    Initial: {initial ? <CheckCircleIcon className="text-green-500" /> : <CancelIcon className="text-red-500" />}
                  </p>
                </div>
                <div className="flex flex-col items-center">
                  <h4 className="font-epilogue font-semibold text-[14px] text-[#b2b3bd] leading-[22px]">
                    <PeopleAltIcon className="text-md" /> {people}
                  </h4>
                </div>
              </div>
              ...
              <div className="flex justify-between w-[100%] items-center mt-[20px] gap-[12px]">
                <p className="flex-1 font-epilogue font-normal text-[12px] text-[#808191] truncate">
                  by <span className="text-[#b2b3bd]">{by.slice(0, 6)}...${by.slice(-4)}</span>
                </p>
                {paid ? (
                  <button className="hover:bg-blue-900 bg-blue-300 text-white px-3 rounded">Pay</button>
                ) : (
                  <button onClick={sendETH} className="hover:bg-blue-900 bg-blue-800 text-white px-3 rounded">Pay</button>
                )}
              </div>
            </div>
          </div>
...
const Card = ({ id, clickJoin, title, desc, people, amount, by, initial }) => {
  return (
    <>
      {title !== "none" && (
        <div className="sm:w-[288px] w-[100%] rounded-[15px] bg-slate-50 cursor-pointer">
          <div className="flex p-4">
            <div>
              <div className="block">
                <Link to={`/chit/${id}`}>
              ...
              <div className="flex justify-between w-[100%] items-center mt-[20px] gap-[12px]">
                ...
                <button onClick={() => clickJoin(id)} className="hover:bg-blue-900 bg-blue-800 text-white px-3 rounded">
                  <AddIcon /> Join
                </button>
              </div>
            </div>
      ...
      <div className="w-screen">
        ...
        <div className="flex justify-evenly w-[100%] mt-2">
          {myChits.length > 0 ? (
            ...
            <p className="text-center text-black">None</p>
          )}
        </div>
      </div>
